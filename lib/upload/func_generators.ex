defmodule GameriaStorage.Upload.FuncGenerators do
    ...
    |> case do
      ...
      :default ->
        [
          quote(
            do:
              def(upload_path(%{uploader_key: uploader_key} = _struct),
                do: "/uploads/users/#{uploader_key}/"
              )
          )
        ]
  ...
  def gen_size_limit_func(size_limit) do
          ...
          [
            quote do
              defp validate_file_size(
                     %Ecto.Changeset{valid?: true, changes: %{file_size: file_size}} = changeset
                   )
                   when file_size < max and file_size > min,
                   do: changeset
            end
          ]
          |> Macro.prewalk(fn
    ...
    {:__block__, _, def_list} =
      quote do
        defp validate_file_size(%Ecto.Changeset{valid?: true} = changeset),
          do: changeset |> add_error(:file_size, "size not acceptable")
